...
class ItineraryService
{
    ...
    public function recalculateAfterReorder(
        ...
        foreach ($items as $item) {
            ...
            // ⚡ Bolt: Prepare data for bulk upsert to avoid N+1 update queries
            $upsertData[] = [
                'id' => $item->id,
        ...
        if (!empty($upsertData)) {
            // Bulk upsert to update all items in a single query
            ItineraryItem::upsert(
                $upsertData,
                ['id'], // Unique key to match
                ['sequence_order', 'dist_from_prev_km', 'est_transport_cost', 'transportation_mode'] // Columns to update
            );
        }
