...
export default function ItineraryCard({
    ...
    const zoneStyle = zoneColors[destination?.zone?.id] || {
    ...
    const ticketVariants =
        destination?.ticket_variants || destination?.ticketVariants || [];
    const mandatoryTicket = ticketVariants.find((t) => t.is_mandatory);
    const optionalTickets = ticketVariants.filter((t) => !t.is_mandatory);

    return (
        <div
            className={`relative glass-card rounded-2xl transition-all duration-300 group overflow-hidden ${
                isDragging
        ...
        >
            {/* Left Border accent */}
            <div
                className={`absolute top-0 bottom-0 left-0 w-1.5 ${zoneStyle.bg.replace("bg-", "bg-gradient-to-b from-").replace("50", "400").replace("900/20", "500")}`}
            />
            ...
            <div className="p-4 pl-6">
                ...
                <div className="flex items-start gap-4">
                    ...
                    >
                        <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-sm ${zoneStyle.bg} ${zoneStyle.text} ring-1 ring-inset ring-black/5`}
                        >
                            {index + 1}
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs mb-3">
                            {/* Zone Badge */}
                            <span
                                className={`px-2.5 py-1 rounded-lg font-medium ${zoneStyle.bg} ${zoneStyle.text} ring-1 ring-inset ring-black/5`}
                            >
                        ...
                        {(mandatoryTicket || optionalTickets.length > 0) && (
                            <div className="bg-gray-50/50 dark:bg-gray-800/30 rounded-xl p-3 border border-gray-100 dark:border-gray-700/50">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm font-bold text-gray-900 dark:text-white">
                                                    Rp{" "}
                                                    {mandatoryTicket.price.toLocaleString(
                                                        "id-ID",
                                                    )}
                                                </p>
                                            </div>
                                        ...
                                        {optionalTickets.length > 0 && (
                                            <button
                                                ...
                                                aria-controls={ticketContentId}
                                                aria-label={
                                                    isExpanded
                                                        ? "Tutup opsi tiket tambahan"
                                                        : `Lihat ${optionalTickets.length} opsi tiket tambahan`
                                                }
                                ...
                                {isExpanded && optionalTickets.length > 0 && (
                                    <div
                                        id={ticketContentId}
                                        className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700/50 space-y-2"
                                    >
                                        ...
                                        {optionalTickets.map((ticket) => (
                                            ...
                                            >
                                                <span className="text-xs text-gray-600 dark:text-gray-300 group-hover/ticket:text-gray-900 dark:group-hover/ticket:text-white transition-colors">
                                                    {ticket.name}
                                                </span>
