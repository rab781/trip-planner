...
## 2024-05-25 - [Critical] Direct usage of env() outside config files
**Vulnerability:** The application used `env('CHUTES_API_TOKEN')` directly in `ChatController.php`. If configuration caching is enabled (`php artisan config:cache`), `env()` calls outside of config files return `null`. This could lead to sending empty API tokens to third-party services, potentially bypassing intended authentication, causing unintended service access, or exposing internal error details when the API rejects the request.
**Learning:** In Laravel, the `env()` helper is only guaranteed to return correct values when called from within `config/*.php` files. Outside of configuration files, we must use the `config()` helper to access those values securely and reliably.
**Prevention:** Always define external API keys or environment variables in config files (e.g., `config/services.php`) and access them via the `config()` helper.
