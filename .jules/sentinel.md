...
## 2025-02-14 - [Medium] Direct env() usage outside configuration files
**Vulnerability:** Retrieving API credentials directly via `env('CHUTES_API_TOKEN')` in `ChatController.php` instead of using the `config()` helper.
**Learning:** In Laravel, executing `php artisan config:cache` prevents `env()` from returning values for anything outside the configuration files, causing it to return `null`. This can lead to application crashes, unexpected behavior, or security risks where empty credentials might bypass intended validation or expose error structures.
**Prevention:** Always declare external API keys or secrets in configuration files (like `config/services.php`) and access them using the `config()` helper throughout the application logic.
