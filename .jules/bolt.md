...
## 2026-04-02 - N+1 in bulk request processing loops via find()
**Learning:** Making `Model::find($id)` calls inside a loop during bulk resource creation (like creating many itinerary items at once) causes an N+1 query bottleneck because each loop iteration triggers a separate database lookup.
**Action:** Always pre-fetch related models outside of loops using `Model::whereIn('id', $ids)->get()->keyBy('id')` and then access them using `$map->get($id)` to avoid hidden query costs.