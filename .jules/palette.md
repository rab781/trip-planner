...
## 2026-06-23 - View Toggle Buttons Accessibility
**Learning:** For frontend accessibility, view toggle buttons (like switching between 'map' and 'list' views) that rely on visual styling to indicate the active state need an explicit way to communicate this to screen readers.
**Action:** Always include the `aria-pressed={condition}` attribute on view toggle buttons to convey the selected state to screen readers.
