...
## 2024-05-30 - View Toggle Accessibility (aria-pressed)
**Learning:** For frontend accessibility, when implementing view toggle buttons (e.g., switching between 'map' and 'list' views) that rely on visual styling to indicate the active state, screen readers cannot interpret the active state purely from CSS classes.
**Action:** Always include the `aria-pressed={condition}` attribute to convey the selected state to screen readers on view toggle buttons.
