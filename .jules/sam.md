...
## 2026-07-23 - Fix API Code Example in README
**Learning:** Found a code example in the README that was both factually incorrect based on the text description (fetching from a public endpoint when it claimed to require auth) and technically incomplete (missing the Bearer token entirely). Additionally, discovered that the `php artisan route:list` command failed because the API route file used controller aliases (e.g., `AdminDestinationController`) that were not imported or resolved correctly.
**Action:** Always verify that code examples in documentation match the actual API implementation and the accompanying descriptive text. Specifically, ensure that examples claiming to require authentication explicitly show the  header. Furthermore, when discovering broken framework tooling like route lists due to missing imports, fix the route definitions by using the fully qualified class names (FQCN) to ensure stability.
## 2026-07-23 - Fix API Code Example in README
**Learning:** Found a code example in the README that was both factually incorrect based on the text description (fetching from a public endpoint when it claimed to require auth) and technically incomplete (missing the Bearer token entirely). Additionally, discovered that the php artisan route:list command failed because the API route file used controller aliases (e.g., AdminDestinationController) that were not imported or resolved correctly.
**Action:** Always verify that code examples in documentation match the actual API implementation and the accompanying descriptive text. Specifically, ensure that examples claiming to require authentication explicitly show the Authorization header. Furthermore, when discovering broken framework tooling like route lists due to missing imports, fix the route definitions by using the fully qualified class names (FQCN) to ensure stability.
